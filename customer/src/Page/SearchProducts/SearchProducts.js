...
const SearchProducts = () => {
    ...
    const fetchDataProduct = async (page) => {
        try {
            const response = await axios.get(`/api/customer/products/search/${decodeURIComponent(keyword)}?page=` + page);
    ...
    return (
        <div className={cx("Product-with-subcategory")}>
            ...
            <div className={cx("title")}>
                <p>Tìm thấy {products.length} sản phẩm theo từ khóa "<strong>{decodeURIComponent(keyword)}</strong>" </p>
            </div>
