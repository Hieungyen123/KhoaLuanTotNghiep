                ...
                <div className={cx("Content")}>
                                ...
                                {Context.customer === undefined || Context.customer === null
                                    ...
                                    : (
                                        <div onClick={() => { setOpen(false) }} className={cx('user-bar')}>
                                            <Link to={'/myprofile/profile/' + Context.customer._id}> <img src={Context.customer.image.path || imgUser} alt="" /> </Link>
                                            <p>{Context.customer.username || Context.customer.name}</p>
                                        </div>
                                    )
                                }
                    ...
                    <div className={cx("Bottom")}>
                        ...
                        <div className={cx("Input")} >
                            ...
                            <Link to={'/search/' + valueSeacrch}><SearchOutlinedIcon className={cx("icon-search")}></SearchOutlinedIcon></Link>
                                ...
                                <div className={cx("Input-search-result")}>
                                    ...
                                    <div className={cx("Input-search-result-List")}>
                                        {resultSearch.length > 0 ? resultSearch?.map(item => {
                                            return (
                                                <Link to={'/product/' + item._id} key={item._id} className={cx("item")}>
